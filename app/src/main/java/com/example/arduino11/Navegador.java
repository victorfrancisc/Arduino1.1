                ...
                try {
                    ...
                    if (row.length() > 0) {
                        for (int p = 0; p < row.length(); p++) {
                            ...
                            incendio  = Integer.valueOf(aux.get("opcion1").toString());


                        }
                    }
                    if (incendio > 160 || gas >= 75) {
...
public void alertas(int temperatura,int gas)
{
    ...
    builder.setMessage("Valores elavados de temperatura:"+String.valueOf(temperatura)+" "+"valor de gases:"+String.valueOf(gas))
            .setTitle("posi").setPositiveButton("Cerrar", new DialogInterface.OnClickListener() {
    @Override
